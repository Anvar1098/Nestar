...
export class SocketGateway implements OnGatewayInit {
  ...
  handleConnection(client: WebSocket, ...args: any[]) {
    ...
    this.emitMessage(infoMsg);  // yangi ulanganlaga shu xabar yetib boradi!
  }


  handleDisconnect(client: WebSocket) {
    this.summaryClient--;
    this.logger.verbose(` Disonnection & total [${this.summaryClient}]`);
  ...
  public async handleMessage(client: any, payload: string): Promise<void> {
    const newMessage: MessagePayload = { event: 'meesage', text: payload }
